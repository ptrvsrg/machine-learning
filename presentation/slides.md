...
<div class="pt-4 pb-8 text-sm opacity-80">
...
#### Что значит «оптимизация по градиенту»


<div class="pt-4 text-sm opacity-80">

- задаётся функция потерь
- на каждой итерации вычисляется градиент функции потерь по текущим предсказаниям модели
- антиградиент указывает направление уменьшения ошибки
- очередное дерево обучается так, чтобы аппроксимировать этот антиградиент
- затем предсказание модели обновляется добавлением нового дерева с некоторым шагом
